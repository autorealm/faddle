...
class Captcha {
	...
	function __construct() {
		$rand = rand(0,1);
		if($rand==0) {
			$this->font = RES_DIR .DS.'fonts'.DS.'elephant.ttf';
		} else {
			$this->font = RES_DIR .DS.'fonts'.DS.'vineta.ttf';
		}
	}
	...
	public function doimage() {
		...
		if (!$this->font_color) {
			...
			$this->font_color = imagecolorallocate($this->img, hexdec(substr($this->font_color, 1,2))
				, hexdec(substr($this->font_color, 3,2))
				, hexdec(substr($this->font_color, 5,2)));
		}
		//设置背景色
		if ($this->background)
		$background = imagecolorallocate($this->img,hexdec(substr($this->background, 1,2))
			,hexdec(substr($this->background, 3,2))
			,hexdec(substr($this->background, 5,2)));
		else 
		$background = imagecolorallocate($this->img, mt_rand(200, 255), mt_rand(200, 255), mt_rand(200, 255));
		//画一个柜形，设置背景颜色。
	...
	private function creat_font() {
		...
		for ($i=0; $i<$this->code_len; $i++) {
			imagettftext($this->img, $this->font_size, rand(-30,30), $x*$i+rand(0,5), $this->height/1.4
				, $this->font_color, $this->font, $this->code[$i]);
