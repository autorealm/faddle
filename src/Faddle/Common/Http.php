...
class Http {
	...
	function request($url, $referer = '', $limit = 0, $timeout = 30, $block = TRUE, $readfs = TRUE) {
		$matches = parse_url($url);
		$host = $matches['host'];
		$path = $matches['path'] ? $matches['path'].($matches['query'] ? '?'.$matches['query'] : '') : '/';
		$port = isset($matches['port']) ? $matches['port'] : 80;
		$ua = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'Faddle HttpClient/0.10.1';
		...
		if(!$fp) {
			...
			if (!$status['timed_out'] and $readfs) {
