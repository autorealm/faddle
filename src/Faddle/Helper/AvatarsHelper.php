...
class AvatarsHelper {
    ...
    function __construct($Char, $AvatarSize = 256, $FontFile = "") {
        $this->Char            = strtoupper(mb_substr($Char, 0, 1, "UTF-8"));
        $this->AvatarSize      = $AvatarSize;
        $this->Padding         = 30 * ($this->AvatarSize / 256);
        $this->LetterFont      = RES_DIR . '/fonts/SourceCodePro-Light.ttf';
        $this->AsianFont       = RES_DIR . '/fonts/SourceHanSansCN-Normal.ttf';
        ...
        if (is_file($FontFile)) $this->FontFile = $FontFile;
        $this->initialize();
    }
    ...
    public function send($AvatarSize = 0) {
        if (!$AvatarSize) {
