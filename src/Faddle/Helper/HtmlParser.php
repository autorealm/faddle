 ...
 */
class HtmlParser implements \IteratorAggregate {

	const regexp = "/(?P<tag>[a-z0-9]+)?(\[(?P<attr>\S+)=(?P<value>[^\]]+)\])?(#(?P<id>[^\s:>#\.]+))?(\.(?P<class>[^\s:>#\.]+))"
	...
	public function query($expression, $rel = false, $compile = true) {
		return $this->getXpathSubquery($expression, $rel, $compile);
	}
	...
	public function getText($i = 0){
		$array = $this->toArray();
		if(is_array($array[$i]['#text'])) {
			return $array[$i]['#text'][$i];
		} else {
			return $array[$i]['#text'];
		}
	}

	public function toHTML($xpath_element = '') {
		$html = $this->_dom->saveHTML($xpath_element);
		return $html;
	}
