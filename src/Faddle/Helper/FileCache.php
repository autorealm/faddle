...
class FileCache extends Cache {
	...
	public function __construct($options=array()) {
		...
		$this->options['cachedir'] = rtrim($this->options['cachedir'], DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
	...
	private function init() {
		...
		if (!empty($this->options['cachedir']) and !is_dir($this->options['cachedir'])) {
			@mkdir($this->options['cachedir'], 0777);
		}
	}
	...
	public function with($dirname, $mk=true) {
		// 创建应用缓存目录
		$path = $this->options['cachedir'] . $dirname . DIRECTORY_SEPARATOR;
