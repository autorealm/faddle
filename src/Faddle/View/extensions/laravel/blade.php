...
class Blade {
	...
	public static function sharpen()
	{
		Event::listen(View::engine, function($view)
		{
			...
			if ($storage = Blade::storage()) {
				$exists = false; //$storage->exists($compiled);
				//if (! $exists) $storage::$cache[$compiled] = Blade::compile($view);
			} else {
				$exists = file_exists($compiled);
			}
			if ( ! $exists and Blade::expired($view->view, $view->path)) {
				if ($storage) {
					Blade::storage($compiled, Blade::compile($view));
				} else {
	...
	public static function expired($view, $path) {
		...
		if ($storage) {
			//if (array_key_exists(static::compiled($path), (array) $storage::$cache)) return false;
			//echo ' T1: ' . filemtime($path) ."\n" . ' T2: ' .Blade::storage()->time(static::compiled($path));
			return filemtime($path) > Blade::storage()->time(static::compiled($path));
		} else {
	...
	public static function compile_string($value, $view = null)
	{
		...
		if (class_exists('\Blade'))
			$value = \Blade::compile_string($value, $view);
	...
	public static function storage($path=null, $content=null)
	{
		global $blade_storage;
		if (! $blade_storage) return false;
		try {
			if (isset($path)) {
				if (isset($content)) {
					return $blade_storage->put($path, $content);
				} else {
					return $blade_storage->get($path);
				}
			}
		} catch (\Exception $e) {
			return false;
		}
		return $blade_storage;
	}
}
